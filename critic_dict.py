...
class Critic_Dict():
  ...
  def compute_td_err(self, current_state, next_state, reward): # Skal reward tas inn her, eller skal det tas inn ved initialisering?
  ...
  def update_value_dict(self, state, td_err): #Oppdater for å ta inn SAP om nødvendig? 
    self.value_dict[str(state)] += self.learning_rate*td_err*self.eli_dict[self]
  ...
  def update_eli_dict(self, state, td_err, i): # i is index of state in history
    if i == 0:
      ...
      self.eli_dict[str(state)] = self.discount_factor*self.eli_decay*eli_dict[str(state)]
    
  
  #Return value for given state
  def get_value(self, state):
    if str(state) in self.value_dict:
      return self.value_dict[str(state)].get(self.value_dict[str(state)])
    else:
