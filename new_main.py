...
def main():
    ...
    critic = TableCritic(learning_rate=critic_cfg["learning_rate"],
    ...
    for episode in range(training_cfg["number_of_episodes"]):
        ...
        while not env.game_is_finished():
            ...
            for i, sap in enumerate(reversed(path)):
                critic.update_eligs(state=str(sap[0]), i=i)
                critic.train(state=str(sap[0]), td_err=td_err)
                actor.update_eli_dict(state=str(sap[0]), action=(sap[1]), i=i)
                actor.update_policy_dict(state=str(sap[0]), action=(sap[1]), td_err=td_err)
